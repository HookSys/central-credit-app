...
const Login = ({ handleSubmit, entity: { pages } }: TPageProps) => {
  ...
  const onSubmit = async values => {
    const response = await dispatch(
      authRequest(values.get('email'), values.get('password'))
    )
    if (response) {
      setTimeout(() => history.push(company.route))
    }
  }
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className='row justify-content-space-between mt-n2 mb-4 pb-3'>
              <div className='col-12 text-right'>
                <Link className='text-secondary font-weight-regular font-size-md'>
                  Esqueceu a senha?
                </Link>
              </div>
            </div>
