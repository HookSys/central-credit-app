...
router.get('/cedentes', function(req, res) {
  ...
  Cedente.find(params)
    .lean()
    .exec((err, cedentes) => {
      Titulo.find({ D_DATA_PAGAMENTO: null })
        .populate('O_CEDENTE_CNPJ')
        .exec((err, titulos) => {
          cedentes = cedentes.map(ced => {
            let titulosOfCed = titulos.filter(
              titulo => ced.S_CNPJ == titulo.O_CEDENTE_CNPJ.S_CNPJ
            )
            ...
            if (titulosOfCed.length > 0) {
              ...
              ced.B_TITULOS_ATRASADOS =
                titulosOfCed.filter(
                  titulo => new Date(titulo.D_DATA_VENCIMENTO) < new Date()
                ).length > 0
            }
      ...
      Titulo.find({ D_DATA_PAGAMENTO: null })
        ...
        .exec((err, titulos) => {
          sacados = sacados.map(sac => {
            let titulosOfSac = titulos.filter(
              titulo => sac.S_CNPJ == titulo.O_SACADO_CNPJ.S_CNPJ
            )
...
router.get('/titulos', function(req, res) {
  ...
  if (periodo_inicial != null && periodo_final != null)
    params.D_DATA_EMISSAO = { $gt: periodo_inicial, $lt: periodo_final }
