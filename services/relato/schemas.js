...
if (config.selectedCompany === config.COMPANY.CONSULT) {
  mongoose.connect(
    'mongodb://central-credit_01:daniel234@cluster0-shard-00-00-esq5v.mongodb.net:27017,cluster0-shard-00-01-esq5v.mongodb.net:27017,cluster0-shard-00-02-esq5v.mongodb.net:27017/test?replicaSet=Cluster0-shard-0&ssl=true&authSource=admin',
    { useMongoClient: false }
  )
} else if (config.selectedCompany === config.COMPANY.GOLD) {
  mongoose.connect(
    'mongodb://dcfranco:daniel234@homolog-shard-00-00-2ygy8.mongodb.net:27017,homolog-shard-00-01-2ygy8.mongodb.net:27017,homolog-shard-00-02-2ygy8.mongodb.net:27017/test?ssl=true&replicaSet=Homolog-shard-0&authSource=admin',
    { useMongoClient: false }
  )
} else if (config.selectedCompany === config.COMPANY.CENTRAL) {
  mongoose.connect(
    'mongodb://dcfranco:daniel234@francosexperian-shard-00-00-xa0zt.mongodb.net:27017,francosexperian-shard-00-01-xa0zt.mongodb.net:27017,francosexperian-shard-00-02-xa0zt.mongodb.net:27017/test?ssl=true&replicaSet=FrancosExperian-shard-0&authSource=admin',
    { useMongoClient: false }
  )
}
...
const LFILLZEROS = (WORD, SIZE) =>
  WORD.length < SIZE ? Array(SIZE + 1 - WORD.length).join('0') + WORD : WORD
const RFILLZEROS = (WORD, SIZE) =>
  WORD.length < SIZE ? WORD + Array(SIZE + 1 - WORD.length).join('0') : WORD
    ...
    while (i < titulos.length) {
      ...
      if (sacado && sacado[0] && !sacado[0].S_RELACIONADO) {
        relacionados.push({
          ...
          S_TIPO_SACADO: '01',
          S_CLIENTE_DESDE: sacado[0].D_CLIENTE_DESDE
            ? CONVERTDATE(sacado[0].D_CLIENTE_DESDE)
            : BRANCO(8),
      ...
      Titulo.findOneAndUpdate(
        { S_NUMERO_TITULO: titulos[i].S_NUMERO_TITULO },
        { D_ULTIMO_ENVIO: new Date() },
        function(err) {
          if (err) console.log(err)
        }
      )
...
TituloSchema.statics.getRelatoRegs = function(
  periodo_inicio,
  periodo_fim,
  envio = true,
  cb
  ...
  this.find({
    ...
    .exec((err, data) => {
      ...
      let titulos = data.map(item => {
        ...
        if (
          relacionamentos.filter(
            relacionamento => relacionamento.S_SACADO_CNPJ == sacado.S_CNPJ
          ).length == 0 &&
          !sacado.S_RELACIONADO
        ) {
          relacionamentos.push({
            S_IDENTIFICACAO: '01',
            S_SACADO_CNPJ: sacado.S_CNPJ,
            S_TIPO_SACADO: '01',
            S_CLIENTE_DESDE: sacado.D_CLIENTE_DESDE
              ? CONVERTDATE(sacado.D_CLIENTE_DESDE)
              : BRANCO(8),
        ...
        return {
          ...
          S_DATA_EMISSAO: item.D_DATA_EMISSAO
            ? CONVERTDATE(item.D_DATA_EMISSAO)
            : BRANCO(8),
          S_VALOR_TITULO: LFILLZEROS(item.S_VALOR_TITULO, 13),
          D_DATA_VENCIMENTO: item.D_DATA_VENCIMENTO
            ? CONVERTDATE(item.D_DATA_VENCIMENTO)
            : BRANCO(8),
          D_DATA_PAGAMENTO: item.D_DATA_PAGAMENTO
            ? CONVERTDATE(item.D_DATA_PAGAMENTO)
            : BRANCO(8),
      ...
      cb({
        ...
        trailer: {
          S_IDENTIFICACAO: '99',
          S_QUANTIDADE_RELACIONAMENTO: LFILLZEROS(
...
ConfigSchema.statics.getHeaderRelato = function(
  periodo_inicio,
  periodo_fim,
  cb
) {
