    ...
    getFileName(date, only_payments, (data) => {
        if(data.fileName) {
            var mailOptions = {
                ...
                // to: 'dann.c@live.com',
                subject: `Arquivo Serasa ${zeroFill(date_obj.getDate()-1)}/${zeroFill(date_obj.getMonth()+1)}/${date_obj.getFullYear()} - ${config.selectedCompany}`,
...
router.post('/send', function (req, res) {
    ...
    payTitles().then(() => {
        generateAndSendFile(dateFormat(new Date(database), 'mm/dd/yyyy'), only_payments === 'S', (sended) => {
