...
async function getSacado(S_SACADO_CNPJ, cedente, relacionamentos) {
  try {
    ...
    if (sac) return sac._id

    let relacionamento = relacionamentos.filter(
      relacionamento => relacionamento.S_SACADO_CNPJ == S_SACADO_CNPJ
    )
    let sacado = new Sacado({
      S_CNPJ: S_SACADO_CNPJ,
      S_RAZAO_SOCIAL: S_SACADO_CNPJ,
      D_CLIENTE_DESDE: relacionamento[0]
        ? relacionamento[0].S_CLIENTE_DESDE
        : new Date(),
      S_TIPO_CLIENTE: relacionamento[0]
        ? relacionamento[0].S_TIPO_CLIENTE
        : '2',
      S_RELACIONADO: true,
...
async function tituloExists(S_NUMERO_TITULO) {
  try {
    let titulo = await Titulo.findOne({
      S_NUMERO_TITULO: String(S_NUMERO_TITULO)
    }).exec()
...
async function save(titulos, relacionamentos) {
  ...
  for (i = 0; i < titulos.length; i++) {
    ...
    if (!titulo) {
      ...
      let sacado = await getSacado(
        titulos[i].S_SACADO_CNPJ,
        cedente,
        relacionamentos
      )
...
async function load_titles_to_relato(titles, cb) {
  let lines = titles.substr(1, titles.length - 2).split('\\r\\n')

  const { titulos, relacionamentos } = lines.reduce(
    (res, line, i) => {
      ...
      return { titulos, relacionamentos }
    },
    { titulos: [], relacionamentos: [] }
  )
        ...
        if (i != 0 || i != lines.length - 1) {
          if (line.slice(12, 14) == '02') {
            titulos.push({
              ...
              D_DATA_PAGAMENTO:
                line.slice(56, 64).trim() != '' &&
                line.slice(56, 64).trim() != '00000000'
                  ? new Date(
                      line.slice(60, 62) +
                        '/' +
                        line.slice(62, 63) +
...
async function generate_file_relato(
  periodo_inicio,
  periodo_fim,
  quantidade = false,
  cb
            ...
            if (quantidade > 0) {
              ...
              while (titulos.length < quantidade) {
                if (
                  titulos.filter(
                    titulo =>
                      titulo.S_NUMERO_TITULO ==
                      data.titulos[rand].S_NUMERO_TITULO
                  ).length == 0
                ) {
...
async function generate_email_regs(
  quantidade_cedentes,
  quantidade_sacados_por_cedente,
  dateToGenerate,
  cb
) {
  const sacados = []
  ...
  let i = 0
  const cedentesSize = await Cedente.find()
    .count()
    .exec()
  while (i < quantidade_cedentes) {
    let cedenteRand = getRandom(0, cedentesSize - 1)
    let cedente = await Cedente.findOne()
      .skip(cedenteRand)
      .exec()

    if (
      cedentes.filter(ced => ced.id == cedente.id).length == 0 &&
      !cedente.S_DESATIVADO
    ) {
      let j = 0
      let sacadosIn = await Sacado.find({ O_CEDENTE_ORIGIN: cedente })
        .populate('O_CEDENTE_ORIGIN')
        .exec()
  ...
  while (i < sacados.length) {
    let randomValor = schemas.LFILLZEROS(
      getRandom(parseInt(valor_inicio), parseInt(valor_fim)).toString(),
      13
    )
    let randomTitulo = schemas.LFILLZEROS(
      getRandom(parseInt(titulo_inicio), parseInt(titulo_fim)).toString(),
      10
    )
...
async function generate_random_regs(
  periodo_inicio,
  periodo_fim,
  valor_inicio,
  valor_fim,
  ...
  cedente,
  quantidade,
  dias_inicio_vencimento,
  dias_fim_vencimento,
  dias_inicio_pagamento,
  ...
  while (i < quantidade_sacados) {
    ...
    let sacadoRetn = await Sacado.findOne()
      .skip(rand)
      .exec()
  ...
  let dates = Array.from(
    datesBetween(new Date(periodo_inicio), new Date(periodo_fim))
  )
  while (i < parseInt(quantidade)) {
    ...
    let randomDiasVencimento = getRandom(
      parseInt(dias_inicio_vencimento),
      parseInt(dias_fim_vencimento)
    )
    let randomVencimento = addDays(randomDate, randomDiasVencimento)

    let randomDiasPagamento = getRandom(
      parseInt(dias_inicio_pagamento),
      parseInt(dias_inicio_vencimento)
    )
    let datesPagamento = Array.from(
      datesBetween(
        addDays(randomDate, parseInt(randomDiasPagamento)),
        randomVencimento
      )
    )
    let randomPagamento =
      datesPagamento.length > 0
        ? datesPagamento[getRandom(0, datesPagamento.length - 1)]
        : null

    if (
      !(await tituloExists(randomTitulo)) &&
      titulos.filter(titulo => titulo.S_NUMERO_TITULO == randomTitulo).length ==
        0
    ) {
      if (
        pagamentosSkiped < parseInt(quantidade_pagamento) ||
        randomPagamento > new Date()
      ) {
        pagamentosSkiped++
        randomPagamento = null
        if (randomVencimento < new Date()) randomPagamento = randomVencimento
      }
      ...
      titulos.push({
        ...
        S_VALOR_TITULO: randomValor,
        D_DATA_VENCIMENTO: addDays(
          randomVencimento,
          isWeekend(randomVencimento)
        ),
...
async function generate_file_by_titulos(
  periodo_inicio,
  periodo_fim,
  titulos,
  cb
  ...
  await deleteFile(file)
  Config.getHeaderRelato(periodo_inicio, periodo_fim, header => {
    Sacado.getRelacionados(
      titulos,
      async ({ relacionados: relacionamentos }) => {
        await append(file, Object.values(header).join('') + '\r\n')
        ...
        await append(
          ...
          Object.values({
            S_IDENTIFICACAO: '99',
            S_QUANTIDADE_RELACIONAMENTO: schemas.LFILLZEROS(
              relacionamentos.length.toString(),
              11
            ),
            S_BRANCO: schemas.BRANCO(44),
            S_QUANTIDADE_TITULO: schemas.LFILLZEROS(
              titulos.length.toString(),
              11
            ),
...
function getFileName(data_to_generate, only_payments, cb) {
  ...
  if (data_base.getDay() == 1) {
    ...
    date_to_generate = new Date(
      date_to_generate.setDate(date_to_generate.getDate() - 3)
    )
  } else if (data_base.getDay() == 2) {
    first_date = new Date(first_date.setDate(first_date.getDate() - 4))
    date_to_generate = new Date(
      date_to_generate.setDate(date_to_generate.getDate() - 1)
    )
  } else {
  ...
  if (!only_payments) {
    if (
      config.selectedCompany === config.COMPANY.CENTRAL ||
      config.selectedCompany === config.COMPANY.CONSULT
    ) {
      generate_email_regs(2, 2, date_to_generate, data => {
        if (data.success) {
          generate_file_relato(
            first_date.setHours(23, 59, 59),
            last_date,
            false,
            data => {
              cb(data)
...
router.post('/gerarArquivo', function(req, res) {
  ...
  generate_file_by_titulos(periodo_inicio, periodo_fim, titulos, data =>
    res.json(data)
  )
})
...
router.post('/gerarEnvio', function(req, res) {
  ...
  generate_file_relato(periodo_inicio, periodo_fim, quantidade, data =>
    res.json(data)
  )
})
...
router.post('/gerar', function(req, res) {
  ...
  generate_file_relato(periodo_inicio, periodo_fim, false, data =>
    res.json(data)
  )
})
...
router.post('/saveTitles', function(req, res) {
  load_titles_to_relato(req.body, (err, data) => {
...
router.post('/upload', function(req, res) {
  req.files.file.mv(__dirname + '/files_upload/' + req.files.file.name, err => {
    if (err) res.json({ result: false })
      ...
      load_file_relato(
        __dirname + '/files_upload/' + req.files.file.name,
        (err, data) => {
          if (err) res.json({ result: false })
          else res.json({ result: data })
        }
